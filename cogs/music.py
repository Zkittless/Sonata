...
class Music(commands.Cog):
    ...
    async def resolve_spotify(self, url: str) -> list[str]:
        ...
        def _fetch():
            if "track" in url:
                ...
                return [f"{t['name']} {t['artists'][0]['name']}" for t in data["items"][:25]]
            return []
    ...
    async def send_now_playing(self, channel, state, guild):
        song = state.current
        if not song:
            return
    ...
    async def play(self, interaction: discord.Interaction, query: str):
        try:
            ...
            if self.is_spotify_url(query):
                if not self.sp:
                    return await interaction.followup.send(
                        embed=discord.Embed(description="✦ Spotify credentials not configured.", color=PINK_ERROR)
                    )
            ...
            print(f"[PLAY] Got song: {song.title}")
            ...
            try:
                await interaction.followup.send(
                    embed=discord.Embed(description=f"✦ Something went wrong: `{e}`", color=PINK_ERROR)
                )
            except Exception:
                pass
    ...
    async def volume(self, interaction: discord.Interaction, level: int):
        ...
        await interaction.response.send_message(
            embed=discord.Embed(description=f"{SPARKLE} Volume set to **{level}%**", color=PINK_MAIN)
        )
